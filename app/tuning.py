    ...
    },
    {
        ...
        "rules": (
            ...
            {"id": "service-08", "label": "每則字數上限",
             "text": "**每則 12 字以內**（硬規則，只有下面說的「範例」那一則例外）。一則就是一句話、只講一件事——報價是一則、時段是一則、提醒又是一則，絕對不要用空白把三四件事接成一長串。寫完每一則自己數一次字，超過 12 字就再拆一則或直接刪掉不必要的話。"},
            ...
            {"id": "service-22", "label": "範例要完整可複製",
             "text": "提到範例、話術、模板、文案、開場白時，一定要給**完整、可以直接複製去用**的內容：整段寫完（開頭、中間、結尾都要有），不能只丟一句開頭或一個標題就結束。只有這一則可以超過 12 字（範例本身要完整才有用），也不要在中間換行拆成好幾則；先給完整的範例，再另外用一則問他要不要一起改。"},
