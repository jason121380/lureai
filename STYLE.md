...
| `--ok` / `--ok-soft` | `#0b7a5f` / `#e6f4ef` | 正常狀態徽章（軟底上 4.7:1）|
| `--warning` / `--warning-soft` | `#c4281c` / `#fdecea` | 錯誤、轉人工（軟底上 5.0:1）|
| `--caution` / `--caution-soft` | `#b54708` / `#fff3e0` | 警告 |

語意色（ok/warning/caution）只用於狀態徽章與健康檢查，介面主體保持無彩度。

**這幾個值不要調亮回去**：`--muted`、`--ok`、`--warning` 幾乎只用在 12~14px 的
說明字與徽章上，而小字要 4.5:1 才看得清楚（WCAG AA）。原本的 `#8f8f8f` 在白底
只有 3.23:1、`#10a37f` 在 `--ok-soft` 上只有 2.8:1——那是整個介面最看不清楚的
地方，2026-09-02 外部稽核量出來的。要換色請先算對比度。
...
PWA 沒有瀏覽器的返回手勢，選單的展開與收起都要自己做，而且**起手範圍刻意不對稱**：

- 展開／收起都**不限制起手位置**：畫面任何地方往右滑展開、往左滑收起。
- 直向偏移超過 40px 就取消，否則捲清單會誤觸。
- 起手落在 input／textarea／select 或自己會橫向捲動的元素上時不接手（`swipeBlocked()`）——前者橫滑是在移動游標，後者是在捲它自己的內容。

## hover 只給滑鼠

觸控裝置點過之後 `:hover` 會**黏在最後點到的元素上**，側欄就會同時出現兩個灰底（一個是真的選中的，一個是上次點的殘留）。所以：

- **所有 hover 的視覺一律寫在 `@media (hover: hover)` 裡**，新增樣式時不要在外面直接寫 `:hover`。
...
`@media` **不加權重**。同分時是後者勝，所以「手機版覆寫」一定要寫在被覆寫的規則**後面**——寫在檔案前段的 media query 會被後面的桌機規則整組蓋掉，而且沒有任何錯誤訊息。

順帶兩個 flex 的細節：

- 要換行是靠 `flex-basis: 100%`，`order` 只改順序不會換行。
- 換行的元素要縮排時用 `padding` 不要用 `margin`：`margin` 加在 `100%` 之外，右緣會溢出。
...
- **後台字級只有七級**（2026-09-02 收斂）：`13`（mono、徽章、狀態）／`14`（次要說明）／`15`（主體、表單、清單）／`17`（卡片標題）／`20`（區塊標題、彈窗標題）／`24`（頁面標題）／`32`（大數字）。**新增樣式時從這七個裡挑**，不要出現 16、18、21、23——同一種角色卻差 1~2px，看起來只會像沒對齊，不會像層次。
...
- **編號清單要自己記著數到哪**（`renderAssistantMarkup` 的 `orderedNext`）。模型幾乎都會在每個編號底下再寫一段說明，那一段會把清單收掉，下一個編號就落在新的 `<ol>` 裡，而瀏覽器預設每個 `<ol>` 都從 1 開始數——畫面上七個步驟會**全部變成「1.」**（使用者截圖回報）。新開的 `<ol>` 要用 `start` 接上去；遇到標題才歸零。這也順便修掉「模型自己每一項都寫 1.」的情況。`tests/test_markdown_render.py` 守著。
