...
- **2026-09-01 客服模式真人化（使用者細部規格）**：專業穩重親切、語尾不用「啦」可用「唷」；每則訊息不超過 10 個字（講不完拆成下一則，前端 `serviceSentences` 也做安全網拆句）；不用任何標點符號、以空白分段；數字一律阿拉伯數字。回覆像真人打字：第一句直接出現，之後每句停 1~2 秒（`pendingReveal`＋輸入中點點），狀態列／來源／追問全部發完才顯示；串流期間不吐字。逐句泡泡只在模型成功回覆（model_status=used）時啟用，降級知識原文維持一般排版。
...
- **2026-09-01 介面細修一批**：(1) 引用區加「知識來源：」標籤、編號籤縮小（25px 高、15px 圓號）；(2) 空白頁輸入框縮到 640px（手機 400px）、全站輸入列拿掉陰影；(3) 「新對話」不重複疊空白對話——已有空的就切過去，按鈕也拿掉常駐底色；(4) 刪除對話不用瀏覽器原生 confirm，改頁內兩段式（按一下變「確定刪除」，3 秒內再按才刪）。
