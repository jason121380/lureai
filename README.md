...
- 開場建議問題每次隨機（100 題題庫逐題驗證答得出來，`/api/health` 回傳整份打散的題庫、前端每次抽 5 題）
- 專家模式回答一律「一句結論＋動詞開頭的行動條列」，每點附 `[n]` 引用；缺引用會自動重試一次，全形引用自動正規化
- 引用守門只套用在專家模式：客服與 LINE 模式的編號本來就會被剝掉，硬要求會讓正常回覆被丟掉
- 語氣三種：專家模式（條列講深講透、附驗收數字）／客服模式（像真人一則一則傳訊息，逐則顯示成泡泡）／LINE 模式（給 lurebot 用）
- 閒聊、自我介紹、抒發情緒、欲言又止不進檢索，直接讓模型自然接一句，不會回「我手邊的資料不夠」
- 模型降級可觀測：前端顯示降級原因標籤，伺服器 log 有 `[boot]` 開機摘要與 `[llm]` 失敗原因（不含金鑰與問題內容）
- 個資、醫療、法律賠償與勞資話題轉人工
- 管理端知識編輯台：兩大主題總覽、知識庫新增/編輯、品質檢查、**AI 模型校調**、帳號與系統健康
- AI 模型校調：送給模型的規則逐條顯示與編輯（基本規則／三種語氣／閒聊／LINE 出口／固定回覆句），存檔後下一則生效，可比對實際送出的指令
- LINE 大腦外接：lurebot 打 `/api/bot/reply` 取回覆，拆則、去標點、每則間隔秒數都由這裡決定
...
SQLite 是工作資料庫（索引、檢索、當下狀態）。**持久化走 PostgreSQL、不需要 Volume**：在 Zeabur 加一個 PostgreSQL 服務並綁定到本服務（`DATABASE_URL`／`POSTGRES_*` 變數會自動注入），帳號、session、用量稽核、回饋評分、**對話紀錄、個人偏好、AI 校調規則**與後台自訂知識會定期（預設 120 秒、可用 `PG_BACKUP_INTERVAL_SECONDS` 調整）壓成快照存進 Postgres，重新部署時自動還原；知識索引照常由 JSONL 重建，不進快照。沒綁 Postgres 時這些資料只活在容器內，重新部署會歸零。設定 `USER_USERNAME` 與 `USER_PASSWORD` 可在空資料庫自動建立第一個前台帳號，之後也能在管理後台建立或重設帳號。
