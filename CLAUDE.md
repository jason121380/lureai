...
- **回答品質守門**：生成完之後 `app/quality.py` 用程式檢查五件事（整則只有「我陪你拆」、說要給成品卻沒給、要十個只給兩個、問到立場不表態、出現「主管／轉人工／專人」），命中就帶著具體理由重打一次，**第二次還是不合格就送原本那則**——擋掉會變成降級訊息，比廢話更糟。判斷刻意保守，改規則前先讓 `tests/test_quality.py` 的反向案例（正常好回答）全綠。
