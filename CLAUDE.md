...
- **接話一定要補脈絡，不能只看分數**：`is_follow_up()`（`app/service.py`）判斷「這句話裡有沒有店裡的名詞」——沒有名詞就一定要靠前一題才看得懂，而且補脈絡的那份**同分就要贏**（兩個不同 query 的分數不是同一把尺；實測「我想漲價」→「為什麼」兩邊都是 0.8566，嚴格大於會留下無脈絡的那份，選到「客人為什麼會在活動期消費」）。分數只是勉強及格的那條路照舊要嚴格較高，否則自足的問題會被前一題帶走。只看 `WEAK_MATCH_SCORE` 擋不住：「為什麼」0.845、「多少錢」0.898、「太長了」0.851 都高於 0.80，於是完全不補脈絡，撈到的是毫不相干的知識。有名詞的（「燙髮後怎麼整理」「客人說太貴」）自己就撈得準，照舊不補。
...
- **預算要先預留再結算**（`server.BudgetLedger`）：舊版是「先讀餘額 → 呼叫模型 → 事後記帳」，同時進來的請求讀到同一個餘額就一起穿過上限。呼叫前預留一筆估計值（`pricing.typical_cost_twd()`），**記完帳一定要在 finally 裡釋放**，不然那筆估計值會一直掛在帳上把後續請求擋掉。檢查與預留要用 `check_and_reserve` 在同一把鎖裡做完——分開做的話，同微秒到的兩個請求會都讀到同一份 pending、一起穿過上限。
