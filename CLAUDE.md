...
| `app/answer.py`、`app/model_runtime.py` | Responses API 與抽取式降級；`LLM_TIMEOUT_SECONDS` 是單次傳輸上限，`LLM_REQUEST_DEADLINE_SECONDS`（預設 120）是含重試的整體截止時間 |
...
- **自足問題先有來源支持，才補脈絡**：`app/service.py` 的 `WEAK_MATCH_SCORE = 0.80`。自足問題低於此分數時可補前兩題，但只保留當題自己已過政策門檻的來源，且分數嚴格較高才替換。歷史不能讓原本無支持的新主題變成可答。
- **追問不能斷、也不能離題**：建議問題一律經 `FollowupPlanner` 驗證。**「答得出來」跟「跟這一輪有關」是兩件事**（`_answerable` 與 `_on_topic`）——知識庫裡幾乎每一題都答得出來，所以只驗前者等於沒驗，模型寫的三個追問可以全部離題而照樣通過。相關性看的是「這個建議撈到的知識，在不在這一題的鄰域裡」（`_neighbourhood`，這一題自己撈得到的前 24 塊＋答案用到的那幾塊）。政策不擋＋撈得到夠格知識仍是前提，`tests/test_followup_chain.py` 會實跑 50 輪連續追問，任何一輪轉人工就算失敗。挑選順序是：**先從答案實際引用的來源挑尚未問過的策展問題**，再用**「這一題附近還有什麼」**（檢索器的名次，門檻是「跟第一名差不到一成」），**再補「答案用到的那幾塊知識的同分類鄰居」**（只留同分類，用完就少給一個，不要拿別的主題硬湊），**同一塊知識只取一個問法**，最後才動用整份語料的長尾（那條才旋轉，避免長對話繞圈）。**衍生用的來源也要夠接近第一名**：第 4 名常常只是陪襯，拿它的鄰居當建議會整個換主題（實際踩到 `ops-63`「店販開發」被歸在「美髮技術」，於是問賣產品被建議「毛髮構造與三種鏈鍵」）。**頭部湊得到就不要碰長尾**，寧可只給兩個對的。
...
- **斷行要數字數，不是數行數**：客服／LINE 的「每行 12 字、一則 2 行、最多 3 則」在 `static/chat.js` 的 `wrapLine` 與 `app/humanize.py` 的 `wrap_line` 各有一份**同樣規則**的實作，改一邊要改兩邊。只數換行是擋不住的——模型常常回一整行 120 字、一個換行都沒有。**超過 3 則時把中間併成一則，而且一個字都不能掉**（使用者決定：保內容，長話術就是會比較長）。夾過一版行數，實測 8 段收成 3 則時「價格依現場報價」「不要保證一定有效」直接消失——首句、說明與結尾問句都還在，所以畫面看起來是完整的，只有限制與警語不見了。漏掉一句警語會出事，中間那則長一點不會。前端 `serviceSentences` 本來就沒有夾，兩邊一致。三個不能拿掉的細節：字數只算內容不算空白、比較時兩邊都要去空白、先把 2 字以內的碎片黏回後面那段（否則會斷在「20」跟「則」中間）。
- **他開口要一句話術時，第一輪就要給稿**（`tuning.py` 的 `service-29`）：客服模式的「先了解再給指令」是刻意的節奏，但句子裡出現「怎麼說」「怎麼回」「幫我寫」「話術」時，客人已經在等他回覆，那一輪就是他唯一有空看的一輪，只反問等於把他丟在現場（體檢：20 組情境只有 5 組在第一輪就交出可以貼上的話術）。做法固定三則：接住 → 一版可以直接複製的草稿 → 一個會改寫草稿的關鍵問題；只有缺了那件事會讓草稿整個講錯方向（不知道客人有沒有受傷、要不要退費）才可以純反問。**現場緊急（`service-28`）與這條是僅有的兩個例外**，改其中一條時另一條的措辭要跟著看，不要再出現「唯一例外」這種會互相打架的字。
- **回退語不要再掛一次徽章**：客服模式的回退語本身就寫著「這題我手邊的資料不夠 還是你先跟我說一下…」，泡泡下面再長出一顆「這題我先不亂答」，畫面上就是同一句話送了兩次（`static/chat.js` 的 `restatesFallback`）。真人不會在訊息後面補一張狀態標籤。
...
- **回答品質守門**：**驗的是「他最後真的看到的正文」**——分析前要先剝掉 `[n]` 引用（`quality.CITATION_MARK`）**與結尾那幾行 `▷` 建議問題**（`quality.FOLLOWUP_LINE`）。追問行天生就有動詞跟數字（「那我該先改哪一步？」「我需要多少預算？」），整段一起送檢會過關，但服務把追問拆走之後正文只剩一句空話，狀態還是 answered。引用同理——專家模式規定每點都附 `[1]`，不剝的話「我陪你一起拆這個問題 [1]」會被 `NUMBER_PATTERN` 當成「有給數字」放行，整個守門對專家模式等於失效。**引用重試出來的那則也要查**：兩條路的品質檢查都放在「拿到答案」之後、分支之外，補回 `[n]` 只是把格式修好，內容空不空是另一件事。生成完之後 `app/quality.py` 用程式檢查八件事（整則只有「我陪你拆」、說要給成品卻沒給、要十個只給兩個、問到立場不表態、把人推給不存在的對象（「轉人工」「會有專人」——**「主管」不算**，沙龍當然有主管）、被質疑就無條件認錯（道歉收回立場又不給理由；有給理由或重新表態的不算，真的講錯本來就該改口）、同一則自相矛盾（「建議漲價」又「不要漲價」）、他沒提預算卻主動勸「先不要加預算」（來源裡的預算警語是寫給「正想加預算」或「詢問多但預約少」的情境的，問題側放很寬——提到花費、成本、投放、預約、到店都算有脈絡）），命中就帶著具體理由重打一次（**串流與非串流兩條路都有**：網頁走 `chat_stream`，`_enforce_quality` 在串流結束後跑，重打結果由最終 result 覆蓋前端顯示的串流文字），**第二次還是不合格就送原本那則**——擋掉會變成降級訊息，比廢話更糟。判斷刻意保守，改規則前先讓 `tests/test_quality.py` 的反向案例（正常好回答）全綠。
...
- **引用守門**：模型回答每點都要附 `[n]` 引用，而且**編號要真的指得到這一輪的來源**（`AnswerEngine.has_valid_citation`）——只驗形狀的話，「可以先觀察回流率 [99]」在只有 4 塊來源時照樣放行，畫面上就出現一個點不開的來源編號；`_fit_citations` 顯示時也會把指不到來源的編號拿掉。全形引用（【1】（1）〔１〕）會被正規化成 `[1]`，仍缺引用就自動帶警語重試一次（串流與非串流路徑都有，用量兩次都記帳）。**守門只套用在專家模式**：客服模式的編號前端本來就會剝掉、LINE 模式在出口剝掉，硬要求會讓正常回覆被丟掉（客服看到降級訊息，LINE 直接不回話），因此由 `AnswerEngine.requires_citations(tone)` 放行這兩種。改 `app/answer.py` 時勿拆掉 `normalize_citation_marks`、`retry_with_citations` 與 `requires_citations`。
- **知識來源只列答案引用到的**：檢索一次撈三塊，但窄問題常常只有第一塊能用——模型每點都寫 `[1]` 是對的，錯的是把沒被引用的兩塊也掛成「知識來源 2、3」。`app/service.py` 的 `_fit_citations` 在回傳前砍掉沒被引用的來源，並把 `[n]` 重編成連號（引用 1、3 → 列兩則、答案裡的 `[3]` 變 `[2]`）。**只在會顯示編號的語氣做**（`requires_citations`）：客服／LINE 的編號在出口就被剝掉，照樣裁切會一則來源都不剩；一個都沒引用時也不裁，那是引用守門的問題。串流路徑改的是最終 result（前端以 result 為準覆蓋串流文字），所以重編號安全。**客服／LINE 改用分數裁**（`_nearby_citations`，`CITATION_SCORE_RATIO = 0.90`）：那兩種語氣沒有 `[n]` 可以拿來對照，但問「客人嫌剪太短怎麼講」卻列出「私訊要回多長」時，來源標示反而在拆信任。低於第一名九成的那幾塊只是被撈進來陪襯的，不列；**至少留一則**，有回答就要查得到依據。
...
- **省略主題的接話一定要補脈絡，不能只看分數**：`is_follow_up()`（`app/service.py`）只認明確指代或改寫上一則的語法（「為什麼」「然後呢」「再短一點」）。短句或沒有沙龍名詞本身不代表接話。真正接話可沿用前題來源，補脈絡後同分即可替換；自足問題仍須遵守自己的來源支持邊界。
- **他在回答我們自己問的那句話，就不是新主題**（`answers_our_question`／`_supplemented_hits`）：AI 反問、使用者照答、AI 回「這題我先不亂答」是最傷信任的一幕（體檢：10 次回退有 6 次踩在這裡）——「我下午全滿」這種補充本來就不會命中任何一塊知識，只拿它去檢索必然回退。只在**本來就要回退時**（`no_results`／`low_confidence`）才走這條：先用「他原本那一題＋這一則補充」重查，再不行就退回用他原本那一題的來源答。兩個閘門缺一不可——**上一則真的是我們在問他**（句尾的「嗎」「呢」或二選一的「還是」），而且**這一則沒有問句**；帶問句的那一則是新的問題，仍然要自己撐得起來（歷史不能讓沒有來源支持的新主題變成答得出來）。
- **LINE 有自己的時間預算**：`AnswerEngine._timeout_for(tone)` 讓 `line` 語氣夾在 `LINE_TIMEOUT_CEILING`（25 秒）。**打模型的每一條路都要用它**，閒聊那條也是（`smalltalk()` 曾經漏掉，用全域 60 秒，加上停頓最壞超過 reply token 的窗口）。LINE 的 reply token 只有 60 秒，用網頁那套 60 秒 timeout ＋重打 ＋8-25 秒停頓，最壞 150 秒，設計師收到的是已讀不回。
...
- **LINE 出口不硬切正常句子、也不吃掉小數點**：`humanize.postprocess` 只有在單則超過 24 字（兩行的量）且找得到空白時才切，切不到就整句送出——舊版只要超過 10 字就對半硬切，「你這個月私訊大概幾則呢～」會變成兩則、中間還隔 3~5 秒。半形 `.` `:` 只有在夾在數字中間時保留（8.5%、10:30），「！」與「～」是規則明文允許的，兩邊都不剝。**問號不整顆吃掉**（使用者決定）：沒有語助詞收尾的問句把「？」換成「～」（剝掉會變冷冷的陳述句），句尾已有「嗎」「呢」的照舊拿掉——`humanize.soften_questions` 與 `chat.js softenQuestions` 是同一份實作，共用向量守著。
...
- **安全**：全部 SQL 參數化；回應含安全標頭與 HTML CSP（無 inline script）；POST 檢查 Origin；月預算超標即停用模型生成；聊天與登入均有限流。三個不能拆掉的細節：
  ...
  - **assistant 歷史是 client 送上來的，驗不了**。留著是為了接得上「然後呢」，但夾在 `MAX_ASSISTANT_TURNS`（4 則），不讓整個脈絡被假前提佔滿。答案一律來自當下重新檢索的知識。
- **網頁只有 expert 與 service 兩種語氣**（`server.py` 的 `_web_tone`）：`line` 是寫給 LINE 出口的，句子沒有標點、要拆成多則、引用在出口才剝掉，網頁照單全收會出現一段沒有標點的文字，而且跳過了引用守門。
...
- **持久化走 Postgres 快照，不掛 Volume**（使用者決定）：SQLite 是可拋棄的工作庫；`app/replica.py` 把 users／sessions／audits／feedback／自訂知識壓成 gzip JSON 單列快照存 Postgres，開機還原、定期備份（內容沒變不上傳）。改 durable 資料表 schema 時記得欄位取交集的還原邏輯已涵蓋新增欄位，但刪欄位要同步看 `apply_snapshot`。快照是自訂知識的全量真相：還原前先 `clear_custom_chunks()`，只 upsert 的話在別台刪掉的知識會復活。
- **等太久要講一句話**：聊天等待超過 5 秒，三顆點下面會出現分階段提示＋已等秒數（查知識庫 → 整理回答 → 這題比較複雜 → 快好了）。單次傳輸上限 `LLM_TIMEOUT_SECONDS` 預設 60 秒；含重試的整體截止時間由 `LLM_REQUEST_DEADLINE_SECONDS` 控制，預設 120 秒。中間完全沒有交代就跟當掉一樣。**秒數要真的在跳**，只換一句話看久了跟靜止沒兩樣。收到第一段字（客服模式除外，那條路不即時吐字）或整個結束就停。
...
- **健康版本來源**：`build.commit`／`build.timestamp` 只讀建置時注入的 `APP_BUILD_*`，沒注入一律為 `unknown`；規則版本只讀 `tuning.rule_versions(store)`，遷移狀態在 durable `rule_state`，不能放衍生 `app_metadata`。
- **後台定位**：知識編輯台（總覽／知識庫／品質檢查／帳號／系統健康）。知識庫是 QA 式收合清單（一列一則、點標題展開），清單只帶前 400 字，展開時才抓完整內容。搜尋**邊打字邊出結果**（250ms 緩衝、IME 選字中不搜、舊請求後到不蓋新結果），且同時字面比對 locator／chunk_id（搜「ads-10」要找得到，字面命中排最前）。可直接新增編輯知識，存 SQLite（`chunks.origin='custom'`），重建索引不會被覆蓋；`匯出 JSONL` 可下載回存 repo 永久化。
- **LINE 有共同的端到端截止時間**（`server.LINE_TOTAL_BUDGET`＝45 秒）：從收到請求算起，檢索、生成、**每一次重試**與出口的停頓全部從同一份裡扣。舊版是每次呼叫各拿一份完整 timeout（25＋25）再加上 8-25 秒停頓，光名義配置就超過 reply token 的 60 秒窗口。`AnswerEngine.has_time_for_another_call()` 在重試前先問「還來得及嗎」，`humanize.fit_delays()` 把停頓壓進剩下的時間。lurebot 可以用 `budget_seconds` 告訴我們它那邊還剩多少。
- **HTTP 有 worker 上限**（`MAX_WORKERS`，預設 48）：一條連線一條執行緒而且沒有上限的話，慢速連線可以把記憶體與執行緒吃光。滿了直接回 503 不要無限排隊，而且**號誌要在 accept 迴圈（`process_request`）裡、建執行緒之前非阻塞地拿**——在 `process_request_thread` 裡等的話執行緒已經開出去了，等待中的執行緒本身就是爆量。讀 body 另外有總時限（`BODY_READ_TIMEOUT`）——socket timeout 管的是「兩次收到資料之間」，每 25 秒滴一個位元組就能把一條執行緒綁住任意久。
- **解壓限制要在解壓的時候做**（`app/documents.py`）：434 bytes 的 docx 可以展開成 26 萬字，只在最後檢查文字長度時記憶體早就吃掉了。總展開量、單檔展開量、壓縮比、entry 數四道一起看；PDF 的 zlib 用 `decompressobj` 一邊解一邊夾。
- **每次生成都先預留再結算**（`app/budget.py`、`app/model_runtime.py`）：回答、串流、標題、閒聊、文件萃取及每次重試共用 SQL 帳本。預留序列化 UTF-8 bytes＋4096 framing buffer 的輸入上界及有限輸出上限（預設 16384），預算檢查必須包含這一次完整預留。使用者與全站月預算以 Asia/Taipei 計算，0 表示不限預算。全域並發／RPM／TPM 與 429 cooldown 套用所有生成；一個請求（含重試）的 deadline 預設 120 秒，LINE 更短的 deadline 仍有效。provider terminal 收到立刻結算；EOF、斷線及未知用量保留保守費用，不能因為釋放工作槽就歸零。Postgres 模式必須在送出前及結算後同步快照，回呼不得持有 store/runtime 鎖。舊 audits 與新 ledger 每筆費用只算一次。對帳使用 `python3 -m app.reconcile_usage --help`，停機且掌握 provider 帳单證據後才可對 pending 呼叫使用 `--allow-pending`。
- **對話紀錄存伺服器**（2026-09-01 使用者決定改掉原本只存瀏覽器的做法）：`conversations` 表（per user id）＋`user_prefs`（語氣偏好），兩張都在 `DURABLE_TABLES` 裡，換裝置與重新部署都看得到。localStorage 降為離線快取；**每次進入網站（`bootstrap` → `restoreSession`）與分頁回到前景時**都會拉伺服器那份合併（伺服器為主）。兩個不能拆掉的守則：(1) **已同步過之後，「伺服器沒有、本機有」＝在別台刪掉了，不可以再推回去**（推回去會讓刪掉的對話復活），只有上次同步之後才新建的才推，靠 per 帳號的 `lastSyncAt` 標記分辨；(2) **關閉分頁時只有在本機真的有未送出的修改（`pendingPush`）才補送**，否則會用這台的舊版本蓋掉別台剛存的新版本，舊使用者只在瀏覽器裡的紀錄會在第一次登入時自動搬上去。每個帳號最多留 100 段、每段 200 則、單則 2 萬字（`app/server.py` 的 CONVERSATION_* 常數）。稽核仍然照舊另存（問題已遮罩 PII）。**每段對話帶一個 `rev`，而且 dirty 要記到「哪一段」**：
  - 存檔要看狀態碼，而且 **ACK 只能確認它上傳的那一版**。共用一個布林旗標時，A 上傳中、使用者改了 B，A 的成功回覆會把 B 的旗標一起清掉，B 就再也不會被送出去；同一段對話上傳第 1 版途中改成第 2 版也一樣。
  - 伺服器做**版本條件更新**（`storage.save_conversation`，`excluded.rev >= conversations.rev` 才寫）。舊版是「最後一個寫入的人贏」，兩台裝置開著時這台的舊副本可以覆蓋另一台剛存的新版本。
  - **刪除要先立墓碑再送**，而且墓碑要存進 localStorage。只在失敗後才記的話，請求還在路上時剛好同步就會把它合併回來；只放記憶體的話，關掉分頁就忘了，下次登入照樣復活。
  - 合併時比 `rev`：本機還沒送出去的修改不可以被前景刷新拉回來的舊版本蓋掉。
  `tests/test_sync_timing.py` 用受控的 fetch 時序把這四件事跑一次。
- **串流的字要綁定對話**（`static/chat.js` 的 `streamingId`）：delta 回呼不可以直接抓畫面上最後一則訊息。A 還在生成、人切到 B 的話，A 的字會一個一個打進 B 的泡泡裡。最終 result 本來就寫回正確的 conversation 物件，所以只要擋住畫面那一段。
- **編號清單要自己記著數到哪**（`renderAssistantMarkup` 的 `orderedNext`）：模型幾乎都會在每個編號底下再寫一段說明，那一段會把清單收掉，下一個編號落在新的 `<ol>` 裡，瀏覽器預設每個 `<ol>` 都從 1 開始數——畫面上七個步驟全部變成「1.」。新開的 `<ol>` 用 `start` 接上去，遇到標題才歸零。`tests/test_markdown_render.py` 守著。

## 工作流程

- 發布以目前工作分支及 PR 為準；CI 必要工作與外部驗收見 `docs/operations.md`。倉庫 ruleset、正式部署與 MFA 必須查外部系統，不能從 repo 文件推定已啟用。
