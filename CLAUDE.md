...
- **HTTP 有 worker 上限**（`MAX_WORKERS`，預設 48）：一條連線一條執行緒而且沒有上限的話，慢速連線可以把記憶體與執行緒吃光。滿了直接回 503 不要無限排隊。讀 body 另外有總時限（`BODY_READ_TIMEOUT`）——socket timeout 管的是「兩次收到資料之間」，每 25 秒滴一個位元組就能把一條執行緒綁住任意久。
...
- **預算要先預留再結算**（`server.BudgetLedger`）：舊版是「先讀餘額 → 呼叫模型 → 事後記帳」，同時進來的請求讀到同一個餘額就一起穿過上限。呼叫前預留一筆估計值（`pricing.typical_cost_twd()`），**記完帳一定要在 finally 裡釋放**，不然那筆估計值會一直掛在帳上把後續請求擋掉。
