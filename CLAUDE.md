...
- **安全**：全部 SQL 參數化；回應含安全標頭與 HTML CSP（無 inline script）；POST 檢查 Origin；月預算超標即停用模型生成；聊天與登入均有限流。三個不能拆掉的細節：
  - **問題進模型前先遮電話與 Email**（`service.CONTACT_PATTERN` 在 `_validated_question`）。歷史與稽核本來就有遮罩，只有當下這一則是原文送出去的。**不要改用 `SENSITIVE_HISTORY_PATTERN`**，那條含關鍵字的規則對稽核夠好，拿來改寫問題會把「客人一直看手機」也遮掉。
